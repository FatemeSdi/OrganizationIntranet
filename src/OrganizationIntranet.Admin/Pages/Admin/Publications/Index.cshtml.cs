...
public class IndexModel(ApiClient api) : PageModel
{
    ...
    public async Task OnGetAsync() => Result = await api.GetAsync<PublicationListDto>($"api/admin/publications?page={PageNumber}&kind={Uri.EscapeDataString(Kind ?? "")}&search={Uri.EscapeDataString(Search ?? "")}");
}
