...
## پیوست نهایی — تطبیق گزارش‌های تکمیل‌شده سایر تسک‌ها

این پیوست پس از متن اصلی افزوده شد و در موارد زیر بر عبارت‌های «در جریان/Unknown» همان snapshot مقدم است. منابع گزارش‌اند؛ آزمون‌های آن‌ها توسط نویسنده این تحلیل دوباره اجرا نشده است.

- [بازبینی دیتابیس](database-review.md): اجرای واقعی migration سازگاری روی SQL Server، حفظ تعداد داده‌ها، بررسی قیود و تهیه backup همراه VERIFYONLY گزارش شده است. یکتایی سراسری PermissionCode از قبل در بانک وجود داشته و EF با آن منطبق شده؛ ریسک برخورد کد در B0، شکاف نگاشت سورس بود و نباید به‌صورت آسیب‌پذیری اثبات‌شده بانک واقعی بیان شود. زمان‌های تاریخی RolePermission تبدیل نشده‌اند. این نتیجه U008 را برای migration سازگاری می‌بندد؛ وضعیت Publication در گزارش تکمیلی زیر و تغییرات آتی احراز هویت جداگانه پیگیری می‌شود. VERIFYONLY معادل restore کامل نیست.
- [بررسی منوها](menu-verification.md): ۱۸ آزمون موفق و آزمون تعاملی روی HTML/SQLite آزمایشی شامل آیتم فعال، پروفایل، theme، منوی موبایل ۳۹۰، Escape و جستجو گزارش شده است. وضعیت کار منو دیگر active محسوب نمی‌شود. اجرای اصلی restart نشده بود؛ این نتیجه تضمین نسخه در حال اجرای 7231 نیست.
- [ادامه فاز صفر](ui-phase-0-verification.md): صفحات عمومی در عرض‌های ۳۶۰/۷۶۸/۱۴۴۰ بررسی شده‌اند؛ ورود در عرض ۳۶۰ بریدگی فرم دارد و نام پایدار ورودی‌ها نیازمند اصلاح است. این دو یافته به کار احراز هویت تحویل شده‌اند. بنابراین همه آزمون‌های بصری «انجام‌نشده» نیستند، اما پذیرش کامل، نسخه اصلاح‌شده موبایل، پورتال احرازشده و accessibility کامل هنوز باز است.
- اطلاع‌رسانی سازمانی: طبق گزارش تکمیلی مالک تسک «افزودن اطلاع‌رسانی سازمانی» (01a0a960-3193-7af3-a776-c3ffd8ae3146)، SQL عملیاتی اعمال و ۱۱ ستون، ۲ FK و ۴ index تأیید شده‌اند؛ build و ۱۸ آزمون نیز موفق گزارش شده‌اند. تاریخچه خوانده‌شده اجرای اسکریپت up و verify را ثبت می‌کند. بنابراین نصب جدول Publication دیگر پیش‌نیاز انجام‌نشده محسوب نمی‌شود. این نتیجه به گزارش صاحب کار نسبت داده می‌شود و اجرای مستقل این تحلیل نیست. بررسی بصری مرورگر و فعال‌سازی نسخه جدید میزبان‌ها همچنان تأیید نشده‌اند. متن فایل organizational-publications.md در زمان بازخوانی هنوز ادعای قدیمی اجرا نشدن SQL داشت؛ گزارش تکمیلی تسک در این مورد مقدم است.

در نتیجه اولویت عملیاتی پیش از اعلام آمادگی انتشار: پایان و آزمون احراز هویت، فعال‌سازی هماهنگ نسخه جدید سه میزبان پس از build، و بازآزمایی پذیرش باقی‌مانده. این تحلیل هیچ‌یک از این عملیات را اجرا یا مجاز اعلام نمی‌کند.
